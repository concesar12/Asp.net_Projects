...
{
    public class PersonModelBinder : IModelBinder //this interface needs to be implemented so the code below is generated
    {
        public Task BindModelAsync(ModelBindingContext bindingContext)
        {
            Person person = new Person();
            //FirstName and LastName
            if (bindingContext.ValueProvider.GetValue("FirstName").Length > 0) //This will provide that value from the request, return an object of type value provider result
            {
                person.PersonName = bindingContext.ValueProvider.GetValue("LastName").FirstValue; //This is taken again because can be more than one value FirstName to be taken
